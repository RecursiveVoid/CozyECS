...
A small, fast, archetype-based Entity Component System for TypeScript and JavaScript.

- **Fast.** In the [benchmarks](#benchmarks), CozyECS's best form is #1 in all five scenarios
  (median of 5 repeats) against bitecs, wolf-ecs, harmony-ecs, becsy, javelin, miniplex, ecsy,
  geotic and perform-ecs. The lead is 1.1–1.2x in iteration and 1.6–4.1x in structural changes.
- **Compact.** All data lives in typed-array columns, and every archetype table is one buffer.
  100k entities with Position+Velocity take 26 bytes per entity.
- **Typed.** Component schemas give you typed column access (`chunk.col(Position).x` is a
  `Float32Array`) and typed `set`/`get` values.
- **No runtime dependencies**, and the package has no import-time side effects.
...
The package ships ESM (`dist/index.esm.js`), CommonJS (`dist/index.cjs`) and type declarations.
...
> change in any minor release. Full documentation: [docs/GPU.md](docs/GPU.md).
...
- **Calibrate `auto` on the user's machine.** The built-in switch points were
  ...
  [docs/GPU.md, "Choosing a target"](docs/GPU.md#4-choosing-a-target)).
...
More detail: [docs/API.md](docs/API.md) covers iteration and systems, and
[docs/INTERNALS.md](docs/INTERNALS.md) covers storage, entities and the command buffer.
...
1. **Iterate chunks, not entities, and let the library compile the loop.** The fastest forms
   ...
   [Why the plain chunk loop trails closure-constant loops](docs/INTERNALS.md#why-the-plain-chunk-loop-trails-closure-constant-loops).
...
Results from 2026-09-17: Node 22.14, Apple M4. Each job ran in its own process; the tables show
the median of 5 shuffled repeats. See [benchmarks/RESULTS.md](benchmarks/RESULTS.md) for the
full tables, memory, the methodology and the fairness audit.

| scenario | best CozyECS (ops/sec) | best competitor (ops/sec) | paired |
|---|---:|---:|---:|
| packed_5 | 487,645 (systems) / 480,170 (direct) | harmony-ecs 430,455 | 1.12 |
| simple_iter | 415,145 (systems) | bitecs 346,897 | 1.20 |
| frag_iter | 912,828 (systems) / 903,068 (direct) | harmony-ecs 822,210 | 1.13 |
| entity_cycle | 70,947 (direct) | wolf-ecs 16,387 | 4.11 |
| add_remove | 49,691 (direct) | wolf-ecs 31,798 | 1.56 |

"Paired" is the median of (CozyECS's best form ÷ the fastest competitor measured in the *same*
repeat), which cancels out drift in machine load.

Memory for 100k entities with Position+Velocity: CozyECS 26.2 B/entity, harmony-ecs 54.2,
wolf-ecs 72.4, bitecs 302.1, ecsy 978.7.

Not every CozyECS API wins every scenario, and the iteration leads are narrow. On simple_iter, a
raw chunk loop outside a system reached 0.81 of bitecs, and `forEach` led by only 1.04. On
packed_5, harmony-ecs beat every CozyECS form in one of the five repeats. The machine was loaded
during the run, so treat the 1.1–1.2x iteration margins as "comparable to the fastest libraries"
rather than a reliable win; the 1.6–4.1x structural-change margins are well clear of the noise.
RESULTS.md lists every case.

A regression rerun on 2026-09-18 (same machine, more heavily loaded, so only the ratios are
comparable) kept CozyECS first in all five: packed_5 1.02 (a tie with harmony-ecs), simple_iter
1.21, frag_iter 1.06, entity_cycle 4.01, add_remove 1.63. Memory was unchanged.

The GPU / kernel benchmark (`npm run benchmark:gpu`) and its break-even tables are in
[RESULTS.md](benchmarks/RESULTS.md#gpu--kernel) and [docs/GPU.md](docs/GPU.md#4-choosing-a-target).
